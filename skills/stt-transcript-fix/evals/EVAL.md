...
| sonnet | 2026-07-10 | eval-02 | **8/8 PASS** (4트랩 전부 홀드/정규화 정확 — 무근거 인명·병기 명확화·값 모순 보존, Tier-C glossary 미기록) |
| sonnet | 2026-07-10 | eval-03 | run1 7/9 → 2건 분석: ① 마커 배치 신규줄 삽입 = SKILL.md 모호성 → 인라인 append 명문화 후 재실행 **PASS**(45→45 parity) ② '4개=과마킹' = 정답셋 오류(15:03 발화가 (*인사이트_) 트리거 예시와 문자 일치) → JSON 교정. 잔존: 클러스터 dedup run간 변동 ±1 (필수 3마커는 양 run 100%) |
