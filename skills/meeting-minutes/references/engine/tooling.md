...
- **Slack Canvas**: MCP `create_canvas` / `read_canvas` / `update_canvas`.
  ...
  - Flow: create in DM → give URL → report the canvas link; leave channel sharing to the user. **Canvas is a DERIVED artifact — never auto-back-sync canvas edits into the vault canonical** (that inverts the MD-first authority chain and creates a circular dependency with pipeline.md's approval gate: MD approval → vault → canvas). If the user edited the canvas and explicitly asks to reflect it, treat it as a new edit cycle: `read_canvas` → update the work MD → re-approval (new `approved_hash`) → canonical update → re-derive other artifacts.
