...
- **Load evidence via `python "<skill root>/scripts/fixstamp.py" sections "<glossary.md>"`** — prints ONLY §1 (correction table) + §7 (people) + §8 (ownership), ~39% smaller than Read-ing the whole file (§2–§6/§9–§10 are context-only and never used for correction). Do this **once** per run. Only fall back to a raw `Read` of the glossary if `sections` errors ("glossary format changed"). Never re-read §2–§6.
...
- **Before** reading the transcript, run `python "<skill root>/scripts/fixstamp.py" check "<target.txt>" "<glossary.md>"`. **exit 0 = skip everything** — report one line "SKIP (already fixed, unchanged)" and stop for that file (no transcript/glossary Read; measured: without this gate, confirming an already-fixed file wastes ~30k tokens and 2–4 min per run). exit 1 = new file (no stamp), exit 2 = file changed, exit 3 = glossary changed — all mean proceed. exit 4 = path error.
- **Quick-scan**: `quick-scan <target> <glossary>` returns exit 0 if variant density is below threshold (likely clean file, skip full pass) or exit 1 (proceed). Use this BEFORE reading the transcript for large files to avoid wasting reads on clean transcripts. The density threshold is conservative (0.0003) — false-positives are OK (extra pass), false-negatives are NOT (missed corrections).
- **Dry-run mode**: `--dry-run` flag after `check` → prints status without side effects.
- **Batch mode**: `batch <folder> <glossary.md>` checks all `.txt` files in a folder, prints summary with new/changed/skip counts.
- After the correction+marking pass completes (including a confirmed no-change pass), run `... write` to refresh the stamp — **once per file, at the very end** (non-transcript/skipped-by-name files get their `write` immediately at classification). Sidecar: `<target.txt>.fixstamp`. Lockfile `<target.txt>.lock` prevents concurrent edits to the same file.
- Adding glossary rows changes the hash → automatic full re-review (by design — new variants may exist in older transcripts).
- **Fixstamp auto-detects UTF-16** (BOM `FF FE`/`FE FF`) and prints a NOTE.
...
## Grep on Windows (rg fallback)
- Preferred: use the `grep` tool (built-in, regex-capable, no dependency).
- PowerShell fallback: `Select-String -LiteralPath "<file>" -Pattern "<regex>" | ForEach-Object { $_.LineNumber.ToString() + ': ' + $_.Line }`
- **Never assume `rg` is installed** — Windows environments rarely have ripgrep by default.

## Procedure
1. Read the target `.txt` (entire file). **⚠️ Truncation guard**: long meetings exceed the Read token cap (e.g. a 900-line/30k-token file does not fit one Read). Verify the Read reached end-of-file — if truncated, read ALL the rest via offset before starting any correction. Proceeding on a partial read silently leaves the back half uncorrected. **UTF-16 guard**: if Read returns mojibake, the file is likely UTF-16 (BOM `FF FE`/`FE FF`) — the `fix_template.py` helper auto-detects encoding. If writing ad-hoc scripts, read/decode in python and on write re-encode with the ORIGINAL encoding and BOM (phone-app exports are often UTF-16).
2. Load the glossary evidence via `fixstamp.py sections "<glossary.md>"` (§1 correction table + §7 people + §8 ownership only — ~39% cheaper than a full Read). Raw Read only if `sections` reports a format change.
3. **Mask `(*...)` semantic comments first** (identify protected spans) → apply **Tier-A** (below) + confirmed user inline corrections → apply in one batch:
   - **Preferred path: `fix_template.py` helper** (at `scripts/fix_template.py`). Two modes: (A) hardcode `REPLACEMENTS` in script, or (B) pass a JSON file via `--json replacements.json`. JSON format: `{"replacements": [["old", "new", count], ...], "markers": [[line, "(*marker)"], ...], "contextual": [...], "quick_scan": true}`. The script provides: quick-scan guard, auto-backup, count verification, substring-collision detection with word-boundary regex fallback, (*...) comment masking, (문맥) tag filtering, line-count parity, auto-restore, UTF-16 detection, dry-run with line-level diffs, and lockfile concurrency protection.
   - **Quick-scan optimization**: before reading, run `fixstamp.py quick-scan <target> <glossary>`. Exit 0 = low variant density — skip full pass, stamp file, move on. Exit 1 = proceed with correction.
   - **Ad-hoc script path**: ① back up: copy to `.bak` ② verify `text.count(old)` equals the count you collected via Grep ③ batch replace ④ compare `splitlines()` line count, on mismatch restore and abort ⑤ write. **Always wrap in try/except with auto-restore**.
   - **Substring-safety check (required)**: before any replacement, verify that `old` is NOT a substring of `new` and vice versa. If collision exists (e.g. `피던스`⊂`임피던스`), do NOT use `str.replace()` — use regex with word boundaries or context-aware matching instead. `fix_template.py` does this automatically.
   - **Script hygiene (Windows-proven)**: before writing the script, Grep the file for the planned variants to collect ALL occurrences (relying on what you remember from the Read misses lines → post-script stray Edits, rejected-Edit churn, double stamping — measured). **Use ONE Grep with alternation (`변형1|변형2|변형3` as a single regex), not one Grep per variant** — one round-trip returns every hit with line numbers (per-variant Grep loops waste N−1 round-trips of latency+tokens). The script must mask `(*...)` spans BEFORE counting/replacing (a variant occurring inside a span must not match); old-strings must be byte-exact including trailing spaces (real transcripts often have them); save the temp script as UTF-8 and run with `PYTHONUTF8=1` (bare `python` on cp949 consoles corrupts Korean literals — measured count=0 false failures). Temp script location: the Bash tool's `/tmp` works; in PowerShell there is no `/tmp` — use `$env:TEMP` or the target folder. Invoke fixstamp as one plain command line (`python "<path>" check "<a>" "<b>"` — valid in both shells); never wrap it in shell-specific variable syntax.
...
### Density & judgment (conservative — ~10–15 per meeting; **dynamic cap: `max(15, lines // 16)`** — prevents both short-meeting saturation and long-meeting under-marking. Pre-existing user markers count toward cap.)
