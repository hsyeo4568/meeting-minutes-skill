...
> **MD-first approval gate (mandatory, before any external sharing) — enforced by `scripts/mm_run.py`. Post-approve order: SKILL.md §4. Do not load `RUNTIME-PROTOCOL.md` at share unless share-check fails.**
> Order = **① draft MD to the work folder only → ② user review and direct edits → ③ explicit approval → ④ `mm_run approve` (immutable snapshot + lease) → ⑤ dest share-check 0 → create_canvas once → canvas_id → URL → ⑥ phase 6 vault → gmail.** Do not save to the canonical store at draft stage — unapproved drafts get indexed.
...
> Do not mix **draft** MD with canvas/gmail in one turn: users edit the work-folder MD directly. After explicit approval + `mm_run approve`, dest share-check then Canvas then vault + gmail **may run in one breath** (gmail stays draft-only). Create canvas once — ban recrate after approve. The category matrix is output types, not a post-approve serial delay. **Explicit pre-approval escape:** if the user pre-approves in the request ("승인 필요 없이 바로", "사전 승인"), that message IS step ③ — `approve` and hash checks still run. Never canvas/gmail before approve.
...
Parent lookback/materials only. The session agent writes the only body. Do **not** Task a named writer subagent. Do **not** write a full engine minutes then a sibling rewrite file. First work-folder MD = that single file.
...
- Apply writing-principles.md for this draft (same single file). Chronological: `conventions-draft.md` if present, else `conventions.md`. Axis: full `conventions.md`.
...
- **Prerequisite: user approval + `mm_run approve`.** Dest share-check Exit 8 **before** any `create_canvas`. Canvas URL comes **before** phase 6 vault — vault must not delay the link.
...
- **Remap, do not rewrite:** canvas = heading remap (keep 개조식) written to a work-folder `.md`, then after dest share-check 0, `create_canvas(title, content)` **once** with **that file's bytes** (Path read — never retype Korean into tool JSON); vault = copy snapshot body + vault_frontmatter (do not restack); gmail = same facts, 개조식 to 존댓말. Daily envelope from `config.categories.daily.gmail_envelope` / profile `conventions-publish.md` — do not Read a gold sent mail. Regular may use the profile template (cc drift: latest 1 sent, envelope only).
- Branch by channel using the phase 4 category result: per-category `share_md` / `canvas` / `gmail`. Each one is a `gate → create → record → verify` cycle. **Order after approve:** dest `share-check` (`destination` = user, not bot DM, never `user_ids`-only) Exit 8 before create → snapshot Path `create_canvas` once → live `read_canvas` openable → URL → **then** vault. Plan dest is not create dest. 권한 없음 → stop, no Recreate, no URL, no 완료. Vault must not delay the URL. Do not say 완료 until `verify` returns 0.
- Create canvas **exactly once** after dest share-check 0. Ban recrate after approve (orphans + `canvas_creation_failed`). `create_canvas` is title+content only (`dm_user_id` is gone; `user_ids` is record-only). `gate` returns `action: readback` once an id is recorded. Re-share after approve is a blocking manual item, not a second `create_canvas`.
